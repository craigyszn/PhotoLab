...
const BookingConfirmation = () => {
  ...
  const bookingData = location.state?.bookingData;        // from form
  const backendBooking = location.state?.backendBooking;  // from API
  ...
  // If user came here by refreshing / direct link with no state
  if (!bookingData) {
    return (
  ...
  // Use backend bookingId as reference if available
  const bookingRef = backendBooking?.bookingId
    ? `PL-${backendBooking.bookingId.toString().padStart(6, "0")}`
    : `PL-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;

  // Get service details
  const selectedService = services.find(
    (s) => s.id === bookingData.serviceType
  );

  // values from backend (with fallbacks)
  const status = backendBooking?.status || "PENDING";
  const packageType = backendBooking?.packageType || bookingData.serviceType;
  const totalPrice = backendBooking?.totalPrice ?? 5000.0;
  ...
  return (
    <div className="confirmation-page">
      ...
      <div className="confirmation-header">
        ...
        <p>
          Thank you for choosing PhotoLab. We're excited to capture your special
          moments!
        </p>
      </div>
          ...
          <div className="next-steps">
            ...
            <div className="steps-grid">
              ...
              <div className="step-card">
                ...
                <p>
                  Our team will reach out to discuss your vision and
                  requirements
                </p>
              </div>
              <div className="step-card">
                ...
                <h4>Session Day</h4>
                <p>
                  Enjoy your photography session and let us capture your
                  moments
                </p>
              </div>
            </div>
